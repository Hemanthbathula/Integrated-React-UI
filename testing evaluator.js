...
class Evaluator extends Component {
    ...
    setUpDb = () => {
        ...
        }).catch(error => this.setState({ erMessage : error.response  ? error.response.data.message : error.message}))
    }
        ...
        return (
            <div>
                ...
                {this.state.erMessage ? (
                    <div className="col-md-5 offset-3">
                        <div className='alert alert-danger alert-dismissible' id="alert">
                            <button type="button" className="close" data-dismiss="alert">&times;</button>
                            <strong>{this.state.erMessage}</strong>
                        </div>
                    </div>
                ) : null}
